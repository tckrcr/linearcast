...
export function ChannelGuide({ activeChannelID, onSelect }: Props) {
  ...
  const nextDisabled = maxScheduleEndMs > 0 && windowEndMs >= maxScheduleEndMs;

  return (
    <div className="channel-guide">
      <div className="channel-guide-controls">
        ...
        <div className="schedule-nav">
          ...
          <button
            type="button"
            onClick={() => shiftWindow(windowHours)}
            disabled={nextDisabled}
            title={nextDisabled ? "no schedule built beyond this point yet" : undefined}
          >
            next →
          </button>
        </div>
      ...
      {channels.length === 0 ? (
        <p className="muted">
          {loading ? "loading guide…" : "No channels yet — import one from the Admin panel."}
        </p>
      ) : (
        <div className="guide-scroll">
          <div className="guide-grid" style={{ gridTemplateColumns: `${LABEL_WIDTH_PX}px 1fr` }}>
            ...
            <div className="guide-rail" style={{ height: `${RAIL_HEIGHT_PX}px` }}>
              {ticks.map((tickMs, idx) => (
                <Fragment key={tickMs}>
                  <div className="schedule-timeline-tick" style={{ left: `${msToPct(tickMs)}%` }} />
                  {idx < ticks.length - 1 && (
                    <div
            ...
            {channels.map((channel) => (
              <Fragment key={channel.id}>
                <button
                  type="button"
                  className={`guide-channel-label${channel.id === activeChannelID ? " is-active" : ""}`}
                  style={{ height: `${ROW_HEIGHT_PX}px` }}
                  onClick={() => onSelect(channel.id)}
                  title={`Tune in to ${channel.displayName || channel.id}`}
                >
                ...
                <div className="guide-track" style={{ height: `${ROW_HEIGHT_PX}px` }}>
                  {renderChannelBlocks(channel, windowStartMs, windowEndMs, nowMs, msToPct, onSelect)}
                </div>
              </Fragment>
            ))}
