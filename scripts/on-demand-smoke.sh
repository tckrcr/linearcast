...
# on-demand-smoke.sh — formal on-demand session drills
#
# Prerequisites:
...
#
# Usage:
#   scripts/on-demand-smoke.sh [--password <pw>] [--host <host>]
...
usage() {
  cat <<'EOF'
Usage: scripts/on-demand-smoke.sh [options]

Options:
  --password <pw>     Admin password (default: from env ADMIN_PASSWORD)
  --host <host>       Host address (default: localhost)
  --port <port>       Port (default: 8080)
  --timeout <sec>     Max seconds to wait for session readiness (default: 120)
...
save_settings() {
  ORIG_MAX_CONCURRENT=$(api_get "$BASE/api/admin/on-demand-session-settings" | jq '.maxConcurrent')
  ORIG_GRACE_SECONDS=$(api_get "$BASE/api/admin/on-demand-session-settings" | jq '.graceSeconds')
}

restore_settings() {
  if [[ -n "$ORIG_MAX_CONCURRENT" ]]; then
    api_put "$BASE/api/admin/on-demand-session-settings" \
      "$(api_get "$BASE/api/admin/on-demand-session-settings" | jq ".maxConcurrent = $ORIG_MAX_CONCURRENT | .graceSeconds = $ORIG_GRACE_SECONDS")" \
      >/dev/null 2>&1 || true
  fi
}
...
drill_warming() {
  local channel="$1"
  echo "--- Drill 2: Warming 503 → session ready (channel: $channel) ---"

  local manifest_url="$BASE/channel/$channel/stream.m3u8"
  local seg_playlist_url="$BASE/channel/$channel/packaged/h264-main-1080p/stream.m3u8"
...
drill_metrics() {
  ...
  local missing=()
  for m in linearcast_on_demand_session_spawns_total \
           linearcast_on_demand_session_restarts_total \
           linearcast_on_demand_session_evictions_total \
           linearcast_on_demand_warming_503_total \
           linearcast_on_demand_at_capacity_503_total \
           linearcast_on_demand_session_spawn_latency_seconds \
           linearcast_on_demand_sessions; do
  ...
  if [[ ${#missing[@]} -eq 0 ]]; then
    ...
    spawns=$(echo "$body" | awk '/^linearcast_on_demand_session_spawns_total /{print $2}')
    restarts=$(echo "$body" | awk '/^linearcast_on_demand_session_restarts_total /{print $2}')
    evicts=$(echo "$body" | awk '/^linearcast_on_demand_session_evictions_total /{print $2}')
  ...
  starting=$(echo "$body" | awk '/^linearcast_on_demand_sessions{state="starting"} /{print $2}')
  serving=$(echo "$body" | awk '/^linearcast_on_demand_sessions{state="serving"} /{print $2}')
  failed=$(echo "$body" | awk '/^linearcast_on_demand_sessions{state="failed"} /{print $2}')
...
drill_grace_idle() {
  ...
  local cur
  cur=$(api_get "$BASE/api/admin/on-demand-session-settings")
  api_put "$BASE/api/admin/on-demand-session-settings" \
    "$(echo "$cur" | jq '.graceSeconds = 5')" >/dev/null
  echo "  Grace set to 5s"

  # Touch the channel by fetching manifest
  curl -sf "$BASE/channel/$channel/stream.m3u8" >/dev/null
  echo "  Channel touched"

  # Restore grace and wait for sweep (default grace + sweep interval + margin)
  api_put "$BASE/api/admin/on-demand-session-settings" \
    "$(api_get "$BASE/api/admin/on-demand-session-settings" | jq ".graceSeconds = $ORIG_GRACE_SECONDS")" >/dev/null
  echo "  Grace restored to ${ORIG_GRACE_SECONDS}s"
  sleep 15
  local serving
  serving=$(curl -fsS "$BASE/metrics" | awk '/^linearcast_on_demand_sessions{state="serving"} /{print $2}')
  serving=$((serving + 0))
  echo "  serving sessions: $serving"
  if [[ "$serving" -eq 0 ]]; then
    echo "all sessions cleaned up"
    pass
  else
    # Not a hard failure — other channels may have active sessions
...
drill_ffmpeg_kill() {
  ...
  echo -n "  Warming up session... "
  curl -sf "$BASE/channel/$channel/stream.m3u8" >/dev/null
  sleep 5
  local segcount
  segcount=$(curl -sf "$BASE/channel/$channel/packaged/h264-main-1080p/stream.m3u8" 2>/dev/null | grep -c '\.m4s' || true)
  segcount=$((segcount + 0))
  if [[ "$segcount" -eq 0 ]]; then
    fail "no active session"
    return 1
  fi
  echo "session active ($segcount segments)"

  # Read restart count before kill
  local before
  before=$(curl -fsS "$BASE/metrics" | awk '/^linearcast_on_demand_session_restarts_total /{print $2}')
  before=${before:-0}
  ...
  while [[ $waited -lt 30 ]]; do
    ...
    segcount=$((segcount + 0))
    local after
    after=$(curl -fsS "$BASE/metrics" | awk '/^linearcast_on_demand_session_restarts_total /{print $2}')
    after=${after:-0}
  ...
  if $recovered; then pass; else fail "session did not recover"; return 1; fi
}
...
drill_max_concurrent_eviction() {
  ...
  # Fetch the first channel to start a session
  local c1 c2
  ...
  # Start session on c1
  curl -sf "$BASE/channel/$c1/stream.m3u8" >/dev/null
  sleep 3
  local c1_segs
  c1_segs=$(curl -sf "$BASE/channel/$c1/packaged/h264-main-1080p/stream.m3u8" 2>/dev/null | grep -c '\.m4s' || true)
  c1_segs=$((c1_segs + 0))
  ...
  local c2_code
  c2_code=$(curl -s -o /dev/null -w '%{http_code}' "$BASE/channel/$c2/stream.m3u8")
  echo "HTTP $c2_code"
  ...
  local waited=0
  while [[ $waited -lt 30 ]]; do
    c2_segs=$(curl -sf "$BASE/channel/$c2/packaged/h264-main-1080p/stream.m3u8" 2>/dev/null | grep -c '\.m4s' || true)
    c2_segs=$((c2_segs + 0))
...
drill_unchanged_packages() {
  ...
  # Touch manifest to ensure session is active
  curl -sf "$BASE/channel/$channel/stream.m3u8" >/dev/null
  sleep 10
  ...
  if [[ "$before" == "$after" ]]; then
    ...
    echo "  BEFORE: $before"
    echo "  AFTER:  $after"
    fail "media_packages changed during on-demand session"
    return 1
  fi
}
...
echo "=============================================="
echo "  On-Demand Session Smoke Drills"
echo "  Target: $BASE"
