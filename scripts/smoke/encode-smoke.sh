...
usage() {
  cat <<'EOF'
Usage: scripts/smoke/encode-smoke.sh <host> <admin-password> [options]
       scripts/smoke/encode-smoke.sh --host <host> (--admin-password <password> | --cookie <cookie>) [options]
       scripts/smoke/encode-smoke.sh --delete-only

Submits the shared smoke media set listed in scripts/smoke/encode-smoke-media.txt for
...
Options:
  --profile <profile>   Package profile to verify (default: h264-1080p-8mbps)
...
fixture_file="scripts/smoke/encode-smoke-media.txt"
...
require_reset_service() {
  ...
  if ! docker compose ps --status running --services linearcast 2>/dev/null | grep -qx "linearcast"; then
    echo "linearcast service is not running; start it with docker compose up -d linearcast" >&2
    exit 1
  fi
}
